...
impl ResourceManager {
    ...
    pub fn get_texture(&mut self, path_to_file: &str) -> Result<Arc<Resource>> {
        ...
        if !self.resources.contains_key(path_to_file) {
            ...
            let id = Uuid::new_v4();
            self.resources
                .insert(id.to_string(), Arc::new(Resource::File(file)));
            let image = image::load_from_memory(&raw_bytes[..]).unwrap();
            let buf_image = image.as_rgba8().unwrap().clone();
            let texture: Arc<Resource> = Arc::new(Resource::Texture(Texture {
                diffuse: buf_image,
                file: id,
            }));
            self.resources.insert(path_to_file.to_string(), texture);
        }
        Ok(self.resources.get(path_to_file).unwrap().clone())
    }
    #[allow(dead_code)]
